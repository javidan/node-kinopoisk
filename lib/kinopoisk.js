...
module.exports =  {
    ...
    movieInfo: function(id, callback){
        ...
        request({url:url, 
            ...
            var result = {
                russian_title: title,
                original_title: $('span[itemprop=alternativeHeadline]').text(),
                year: $('.info td:contains(год) ~ td').text(),
                country: $('.info td:contains(страна) ~ td').text(),
                director: $('.info td:contains(режиссер) ~ td').text(),
                writer: $('.info td:contains(сценарий) ~ td').text(),
                producer: $('.info td:contains(продюсер) ~ td').text(),
                operator: $('.info td:contains(оператор) ~ td').text(),
                composer: $('.info td:contains(композитор) ~ td').text(),
                ...
                description: $('.news .brand_words').text()
            }
